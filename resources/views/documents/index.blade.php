...
<div class="w-full px-4 sm:px-6 lg:px-8 pb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <a href="{{ route('documents.create', ['direction' => 'inbound']) }}" class="inline-flex items-center justify-center bg-white border border-emerald-200 text-emerald-600 hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-300 px-6 py-2.5 rounded-xl text-sm font-semibold shadow-sm transition-all transform hover:-translate-y-0.5">
                <i class="fa-solid fa-inbox mr-2"></i> ลงทะเบียนหนังสือเข้า
            </a>
            <a href="{{ route('documents.create', ['direction' => 'outbound']) }}" class="inline-flex items-center justify-center bg-gradient-to-r from-primary-600 to-primary-500 hover:from-primary-700 hover:to-primary-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-primary-500/30 transition-all transform hover:-translate-y-0.5">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <!-- Toolbar: Tabs & Search -->
        <div class="flex flex-col md:flex-row justify-between items-center border-b border-slate-100 bg-white p-4 gap-4">
            ...
            <div class="flex p-1.5 bg-slate-100 rounded-xl w-full md:w-auto">
                <a href="{{ route('documents.index', ['tab' => 'inbox']) }}" 
                   ...
                   {{ $tab == 'inbox' ? 'bg-white text-slate-800 shadow-sm ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700' }}">
                    <i class="fa-solid fa-inbox {{ $tab == 'inbox' ? 'text-primary-500' : '' }}"></i> หนังสือเข้า
                </a>
                <a href="{{ route('documents.index', ['tab' => 'outbox']) }}" 
                   ...
                   {{ $tab == 'outbox' ? 'bg-white text-slate-800 shadow-sm ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700' }}">
                    <i class="fa-solid fa-paper-plane {{ $tab == 'outbox' ? 'text-primary-500' : '' }}"></i> หนังสือออก
                </a>
            </div>
            ...
            <form action="{{ route('documents.index') }}" method="GET" class="w-full md:w-auto relative group">
                ...
                <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400 group-focus-within:text-primary-500 transition-colors">
                    <i class="fa-solid fa-magnifying-glass"></i>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" 
                    class="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl focus:ring-2 focus:ring-primary-100 focus:border-primary-500 block w-full md:w-80 pl-10 p-2.5 transition-all shadow-sm group-hover:bg-white" 
                    ...
                    <tr class="group hover:bg-slate-50 transition-colors cursor-pointer" onclick="window.location='{{ route('documents.show', $doc) }}'">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                ...
                                @if($receiveNo)
                                    <span class="font-mono font-bold text-emerald-600 text-sm group-hover:text-emerald-700 transition-colors">
                                ...
                                @else
                                    <span class="font-mono font-bold text-slate-700 text-sm group-hover:text-primary-600 transition-colors">
                                        {{ $doc->document_no }}
                                    </span>
                                @endif
                        ...
                        <td class="px-6 py-4">
                            ...
                            @endif
                            <div class="font-semibold text-slate-800 text-base mb-1 line-clamp-2 group-hover:text-primary-700 transition-colors">
                                {{ $doc->title }}
                            </div>
                            @if($doc->attachments->count() > 0)
                                <div class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 text-[10px] font-medium border border-slate-200">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col items-start gap-2">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-white border border-slate-200 text-slate-600 shadow-sm">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    <div class="w-9 h-9 rounded-full bg-white flex items-center justify-center text-xs font-bold text-slate-600 border border-slate-200 shadow-sm overflow-hidden">
                                        @if($doc->user->avatar)
                                            <img src="{{ route('storage.file', ['path' => $doc->user->avatar]) }}" alt="{{ $doc->user->name }}" class="w-full h-full object-cover">
                                        @else
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="flex flex-col items-center justify-center py-16 text-center">
                                ...
                                @if($tab != 'inbox')
                                    <a href="{{ route('documents.create') }}" class="mt-4 inline-flex items-center text-primary-600 hover:text-primary-700 text-sm font-medium hover:underline">
